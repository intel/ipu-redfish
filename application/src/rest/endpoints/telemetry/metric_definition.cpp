...
void endpoint::MetricDefinition::get(const server::Request& request, server::Response& response) {
    ...
    for (const auto& wildcard : definition.get_wildcards()) {
        ...
        wildcard_json[constants::MetricDefinition::VALUES] = json::Json::value_t::array;
        for (const auto& value : wildcard.get_values()) {
            wildcard_json[constants::MetricDefinition::VALUES].push_back(value);
        }
    ...
    for (const auto& value : definition.get_discrete_values()) {
        json[constants::MetricDefinition::DISCRETE_VALUES].push_back(value);
    }
