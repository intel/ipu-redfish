...
static const constexpr char ACTION_NOT_SUPPORTED_MESSAGE[] = "The action %s is not supported by the resource.";
static const constexpr char ACTION_NOT_SUPPORTED_RESOLUTION[] = "Check the Actions property in the resource for the supported actions.";

static const constexpr char URL_TOO_LONG_MESSAGE[] = "The URI %s was not found.";
static const constexpr char INVALID_URI_RESOLUTION[] = "Provide a valid URI and resubmit the request.";

static const constexpr char PAYLOAD_TOO_LARGE_MESSAGE[] = "The supplied payload exceeds the maximum size supported by the service.";
static const constexpr char PAYLOAD_TOO_LARGE_RESOLUTION[] = "Check that the supplied payload is correct and supported by this service.";
...
ServerError ErrorFactory::create_action_not_supported_error(const std::string& action, const std::string& custom_resolution) {
    auto server_error = create_error(BAD_REQUEST, ServerError::ACTION_NOT_SUPPORTED, ::ACTION_NOT_SUPPORTED_MESSAGE, action.c_str());
    auto msg = ::create_message_object(
        ServerError::ACTION_NOT_SUPPORTED,
        server_error.get_message(),
        Severity::Critical,
        custom_resolution.empty()
            ? ::ACTION_NOT_SUPPORTED_RESOLUTION
            : custom_resolution);
    msg.add_message_argument(action);
    server_error.add_extended_message(std::move(msg));
    return server_error;
}

ServerError ErrorFactory::create_uri_too_long_error(const std::string& uri) {
    auto server_error = create_error(URI_TOO_LONG, ServerError::INVALID_URI, ::URL_TOO_LONG_MESSAGE, uri.c_str());
    auto msg = ::create_message_object(
        ServerError::INVALID_URI,
        server_error.get_message(),
        Severity::Critical,
...
ServerError ErrorFactory::create_payload_too_large_error() {
    auto server_error = create_error(CONTENT_TOO_LARGE, ServerError::PAYLOAD_TOO_LARGE, ::PAYLOAD_TOO_LARGE_MESSAGE);
    auto msg = ::create_message_object(
        ServerError::PAYLOAD_TOO_LARGE,
        server_error.get_message(),
        Severity::Critical,
        PAYLOAD_TOO_LARGE_RESOLUTION);
    server_error.add_extended_message(std::move(msg));
    return server_error;
}
