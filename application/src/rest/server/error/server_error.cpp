...
const constexpr char ServerError::ACTION_NOT_SUPPORTED[];

ServerError::~ServerError() {}

ServerError::ServerError(const std::uint32_t status_code) : m_http_status_code(status_code),
...
std::string ServerError::as_string() const {
    return m_error.dump(-1, ' ', true, nlohmann::json::error_handler_t::replace);
}
