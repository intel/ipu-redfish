...
bool Subscription::is_subscribed_for(const Event& event) const {
    ...
    return std::find(m_origin_resources.begin(), m_origin_resources.end(), event.get_origin_of_condition()) != m_origin_resources.end();
}
...
void Subscription::fill_json(json::Json& json) const {
    ...
    json[EventSubscription::EVENT_TYPES] = std::move(event_types_json);
...
bool equal_subscriptions(const Subscription& s1, const Subscription& s2) {
    return s1.get_destination() == s2.get_destination() && s1.get_context() == s2.get_context() && s1.get_event_types().get() == s2.get_event_types().get() && s1.get_origin_resources() == s2.get_origin_resources() && s1.get_name() == s2.get_name() && s1.get_protocol() == s2.get_protocol();
}
