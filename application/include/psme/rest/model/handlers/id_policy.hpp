...
database::IdValue::IdType IdPolicy<CT, NZ>::IdPolicy::get_id(const UuidType& uuid, const UuidType& parent_uuid) {
    ...
    for (;;) {
        ...
        if (id <= last_id.get()) {
            log_error("db",
                      "Internal Error : ID " << _id << " kept in database already allocated, replacing with next that is expected to be free: " << next_candidate_id);
        } else {
            log_error("db",
                      "Internal Error : ID " << _id << " already allocated, replacing with next that is expected to be free: " << next_candidate_id);
        }
