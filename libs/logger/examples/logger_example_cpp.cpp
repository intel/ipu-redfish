...
int main(void) {
    ...
    for (size_t i = 0; i < TEST_COUNT; i++) {
        log_emergency(LOGUSR, "log test"
                                  << " " << i);
        log_alert(LOGUSR, "log test"
                              << " " << i);
        log_critical(LOGUSR, "log test"
                                 << " " << i);
        log_error(LOGUSR, "log test"
                              << " " << i);
        log_warning(LOGUSR, "log test"
                                << " " << i);
        log_notice(LOGUSR, "log test"
                               << " " << i);
        log_info(LOGUSR, "log test"
                             << " " << i);
        log_debug(LOGUSR, "log test"
                              << " " << i);
    }
