...
bool SsdpNotifyPacketValidator::is_valid(const SsdpPacket& packet) const {
    ...
    if (!packet.has_header(SsdpPacket::HOST) || !packet.has_header(SsdpPacket::NT) || !packet.has_header(SsdpPacket::NTS) || !packet.has_header(SsdpPacket::USN)) {
        return false;
    }
...
bool SsdpAlivePacketValidator::validate_subtype(const SsdpPacket& packet) const {
    return SsdpPacket::SSDP_ALIVE_STR == packet.get_header(SsdpPacket::NTS) && packet.has_header(SsdpPacket::CACHE_CONTROL) && (packet.has_header(SsdpPacket::AL) || packet.has_header(SsdpPacket::LOCATION));
}
...
bool SsdpSearchRequestPacketValidator::is_valid(const SsdpPacket& packet) const {
    if (SsdpPacket::Type::SEARCH_REQUEST == packet.get_type()) {
        if (!packet.has_header(SsdpPacket::HOST) || !packet.has_header(SsdpPacket::ST) || !packet.has_header(SsdpPacket::MAN) || !packet.has_header(SsdpPacket::MX)) {
            return false;
        }
...
bool SsdpSearchResponsePacketValidator::is_valid(const SsdpPacket& packet) const {
    if (SsdpPacket::Type::SEARCH_RESPONSE == packet.get_type()) {
        if (!packet.has_header(SsdpPacket::CACHE_CONTROL) || !packet.has_header(SsdpPacket::ST) || !(packet.has_header(SsdpPacket::AL) || packet.has_header(SsdpPacket::LOCATION)) || !packet.has_header(SsdpPacket::USN)) {
            return false;
        }
        return true;
    }
