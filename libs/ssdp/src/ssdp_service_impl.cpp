...
IpAddress SsdpServiceImpl::find_lan_of_peer(const IpAddress& peer) {
    for (const auto& iface : m_network_interfaces) {
        for (const auto& address : iface.second.get_address_list()) {
            ...
            if (peer.get_address_family() == iface_ip.get_address_family() && same_subnet(peer, iface_ip, iface_subnet_mask)) {
                return iface_ip;
            }
        }
    }
...
void SsdpServiceImpl::process_ssdp_m_search(const SsdpPacket& packet,
    ...
    if (st_header == SSDP_ALL_STR || st_header == m_config->get_service_urn()) {
        ...
        auto delayed_msg = msg;
        m_queue.send_or_enqueue(std::move(msg), delay);
        m_queue.send_or_enqueue(std::move(delayed_msg), delay + Ms(DELAY_INCREMENT));
    } else {
        log_debug("ssdp",
...
void SsdpServiceImpl::send_alive() {
    ...
    for (auto& sock : m_notify_sockets) {
        ...
        auto delayed_msg = msg;
        m_queue.send_or_enqueue(std::move(msg));
        m_queue.send_or_enqueue(std::move(delayed_msg), Ms(SCHEDULED_DELAY));
    }
}
...
void SsdpServiceImpl::init() {
    ...
    try {
        ...
        for (const auto& iface : iface_list) {
            if (!is_iface_ignored(iface_names, iface.get_name()) && iface.is_up() && iface.is_running()) {
                try {
...
void SsdpServiceImpl::execute() {
    ...
    auto last_announce_time =
        std::chrono::time_point_cast<Ms>(std::chrono::steady_clock::now()) - check_interval;
