...
bool is_ipv4(const SockaddrStorage& addr) {
    return sizeof(struct sockaddr_in) == addr.addr_len && AF_INET == addr.addr->sa_family;
}

#ifdef NET_HAVE_IPv6
bool is_ipv6(const SockaddrStorage& addr) {
    return sizeof(struct sockaddr_in6) == addr.addr_len && AF_INET6 == addr.addr->sa_family;
}
#endif
