...
NetworkInterface::Type from_arp_hw_type(unsigned arp_hw_type) {
    switch (arp_hw_type) {
    case ARPHRD_ETHER:
        return NetworkInterface::Type::ETHERNET_CSMACD;
    case ARPHRD_IEEE802:
        return NetworkInterface::Type::ISO88025_TOKENRING;
    case ARPHRD_DLCI:
        return NetworkInterface::Type::FRAMERELAY;
    case ARPHRD_PPP:
        return NetworkInterface::Type::PPP;
    case ARPHRD_LOOPBACK:
        return NetworkInterface::Type::SOFTWARE_LOOPBACK;
    case ARPHRD_ATM:
        return NetworkInterface::Type::ATM;
    case ARPHRD_IEEE80211:
        return NetworkInterface::Type::IEEE80211;
    case ARPHRD_TUNNEL:
    case ARPHRD_TUNNEL6:
        return NetworkInterface::Type::TUNNEL;
    case ARPHRD_IEEE1394:
        return NetworkInterface::Type::IEEE1394;
    default:
        return NetworkInterface::Type::OTHER;
    }
}
...
NetworkInterface::AddressTuple address_from_ifaddrs(const ifaddrs* piface) {
    ...
    if (AF_INET == piface->ifa_addr->sa_family
        ...
        std::get<NetworkInterface::IP_ADDRESS>(address) = IpAddress::from_sockaddr(*(piface->ifa_addr));
        std::get<NetworkInterface::SUBNET_MASK>(address) = IpAddress::from_sockaddr(*(piface->ifa_netmask));

        if (piface->ifa_flags & IFF_BROADCAST && piface->ifa_broadaddr) {
            std::get<NetworkInterface::BROADCAST_ADDRESS>(address) = IpAddress::from_sockaddr(*(piface->ifa_broadaddr));
        } else if (piface->ifa_flags & IFF_POINTOPOINT && piface->ifa_dstaddr) {
            std::get<NetworkInterface::BROADCAST_ADDRESS>(address) = IpAddress::from_sockaddr(*(piface->ifa_dstaddr));
        }
    }
    return address;
}
