 ...
 * OptionalField class is based on std::optional class, but with
 * simplified interface resembling that of built-in types rather than smart pointers.
 * */
template <typename T>
class OptionalField : public std::optional<T> {
public:
     ...
     * */
    template <typename V>
    OptionalField(const V& in_value) : std::optional<T>(T(in_value)) {}

    /*!
     * @brief json::Json to OptionalField conversion constructor
     * */
    OptionalField(const json::Json& json) {
        *this = JsonConverter::from_json<T>(json);
    }
    ...
    OptionalField& operator=(const V& rhs) {
        if (!(this->has_value()) || rhs != *this) {
            std::optional<T>::operator=(T(rhs));
        }
        return *this;
    }
...
bool operator==(const OptionalField<T>& lhs, const OptionalField<T>& rhs) {
    return static_cast<const std::optional<T>&>(lhs) == static_cast<const std::optional<T>&>(rhs);
}
...
/*!
 * @brief Logical equals operator for OptionalField holding framework enum object
 * and an underlying base enum value for framework enum object.
 *
 * @param[in] lhs Object on the LHS of the operator.
 * @param[in] rhs Object on the RHS of the operator.
 *
 ...
 * */
template <typename T, typename std::enable_if<model::utils::is_framework_enum<T>::value>::type* = nullptr>
bool operator==(const OptionalField<T>& lhs, const typename T::base_enum& rhs) {
    return (lhs.has_value() ? lhs.value() == rhs : false);
}
 ...
 * */
template <typename T, typename std::enable_if<model::utils::is_framework_enum<T>::value>::type* = nullptr>
bool operator==(const typename T::base_enum& lhs, const OptionalField<T>& rhs) {
    return rhs == lhs;
}
