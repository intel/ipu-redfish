 ...
 *
 * @file iso8601_time_interval.hpp
 */
...
public:
    ...
    public:
         ...
         */
        explicit Exception(const std::string& what_arg) : std::runtime_error("Iso8601TimeInterval - " + what_arg) {
        }
...
private:
    static constexpr std::chrono::milliseconds MAX_MILLIS = std::chrono::hours(std::numeric_limits<int>::max()) + std::chrono::minutes(std::numeric_limits<int>::max()) + std::chrono::seconds(std::numeric_limits<int>::max()) + std::chrono::milliseconds(999);
