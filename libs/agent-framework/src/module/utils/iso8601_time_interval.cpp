 ...
 *
 * @file iso8601_time_interval.cpp
 */
...
Iso8601TimeInterval Iso8601TimeInterval::parse(const std::string& iso8601_interval_str) {
    ...
    if (std::regex_match(iso8601_interval_str, part_match, iso8601_interval_regex)) {
        if (0 != part_match[1].str().compare("T") && 0 != part_match[1].str().compare("t")) {
            ...
            if (!hour_match.empty() || !minute_match.empty() || !second_match.empty() || !fraction_match.empty()) {
                try {
                    auto millis = duration_cast<milliseconds>(hours(digits2int(hour_match))) + duration_cast<milliseconds>(minutes(digits2int(minute_match))) + duration_cast<milliseconds>(seconds(digits2int(second_match))) + milliseconds(digits2int(fraction_match));
