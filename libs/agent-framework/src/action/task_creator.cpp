...
void TaskCreator::set_promised_response(PromisedResponseType promised_response) {
    ...
    auto response_result_callback = [promised_response = std::move(promised_response), task_uuid = std::move(task_uuid)]() {
        TaskResultManager::get_instance()->set_result(task_uuid, promised_response());
    };

    m_task.add_completion_callback(std::move(response_result_callback));
}

void TaskCreator::set_promised_error_thrower(PromisedErrorThrowerType promised_error_thrower) {
    Uuid task_uuid = m_task_resource.get_uuid();

    auto exception_thrower_callback = [task_uuid = std::move(task_uuid), promised_error_thrower = std::move(promised_error_thrower)](const exceptions::GamiException& ex) {
        TaskResultManager::get_instance()->set_exception(task_uuid, promised_error_thrower(ex));
    };

    m_task.add_exception_callback(std::move(exception_thrower_callback));
}
