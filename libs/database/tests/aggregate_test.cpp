...
TEST_F(AggregateTest, ParsingTest) {
    MockPersistentAttributes pa;
    EXPECT_CALL(pa, get_value("uuid", "name", _)).WillOnce(DoAll(SetArgReferee<2>("1.0\037key1=value1\037key2=value2\037MULTIPLE=aggregate\037"
                                                                                  "aggregate1=value3\037aggregate_2=value4\037MULTIPLE=composite"),
                                                                 Return(true)));
...
TEST_F(AggregateTest, DumpTest) {
    MockPersistentAttributes pa;
    EXPECT_CALL(pa, get_value("uuid", "name", _)).WillOnce(DoAll(SetArgReferee<2>("1.0\037key1=valu=e1\037key2=value2\037MULTIPLE=aggregate\037"
                                                                                  "aggregate1=value3\037aggregate_2=value4"),
                                                                 Return(true)))
        .RetiresOnSaturation();
    Aggregate<MockPersistentAttributes> aggr{pa, "uuid", "name"};
    ...
    EXPECT_THROW(aggr.put(std::initializer_list<Aggregate<MockPersistentAttributes>::SinglesContainer::value_type>{{"MULT", "listval1"}, {"dfMULTIPLEas", "listval2"}}), std::runtime_error);
...
TEST_F(AggregateTest, GetAggregatesTest) {
    ...
    EXPECT_CALL(pa, get_value("uuid", "name", _)).WillOnce(DoAll(SetArgReferee<2>("1.0\037key1=valu=e1\037key2=value2\037MULTIPLE=aggregate\037"
                                                                 ...
                                                                 Return(true)))
        .RetiresOnSaturation();
    EXPECT_CALL(pa, get_value("uuid2", "name", _)).WillOnce(DoAll(SetArgReferee<2>("1.0\037key1=valu=e1\037key2=value2\037MULTIPLE=aggregate\037"
                                                                                   "aggregate1=value3\037aggregate_2=value4"),
                                                                  Return(true)))
        .RetiresOnSaturation();
