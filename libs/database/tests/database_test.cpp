...
public:
    DatabaseTester(const std::string& name, bool with_policy = true) : db{Database::create(name, with_policy)} { fdb = static_cast<FileDatabase*>(db.get()); }
...
TEST_F(DatabaseTest, MkdirP) {
    ...
    ASSERT_EQ(std::filesystem::status(dir_path).permissions(), mode);
    ...
    // Test case where path is a file
    char tmp_file[] = "/tmp/file_XXXXXX";
    int fd = mkstemp(tmp_file);
    ASSERT_NE(fd, -1);
    close(fd);
    ASSERT_FALSE(FileDatabase::mkdir_p(tmp_file, mode));
